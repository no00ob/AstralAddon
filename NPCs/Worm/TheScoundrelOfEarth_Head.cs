...
namespace ThumodRe.NPCs.Boss.SoE
{
    [AutoloadBossHead]
    public class TheScoundrelOfEarth_Head : ModNPC
    {
        ...
        {
            DisplayName.SetDefault("Scoundrel of Earth");
        }
        public override void SetDefaults()
        {
            NPC.lifeMax = 250000;        //this is the npc health
            ...
            NPC.buffImmune[BuffID.Cursed] = false;
            ...
            NPC.buffImmune[BuffID.Electrified] = false;
        ...
        public override void BossLoot(ref string name, ref int potionType)
        {
            ...
            Item.NewItem((int)NPC.position.X, (int)NPC.position.Y, NPC.width, NPC.height, ModContent.ItemType<Dirtide>(), 56);
            Item.NewItem((int)NPC.position.X, (int)NPC.position.Y, NPC.width, NPC.height, ModContent.ItemType<HardenedDirtioid>(), 1);
        ...
        public override bool PreAI()
        {
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                if (NPC.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)NPC.Center.X, (int)NPC.Center.Y, Mod.Find<ModNPC>("TheScoundrelOfEarth_Body").Type, NPC.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = NPC.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = NPC.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)NPC.Center.X, (int)NPC.Center.Y, Mod.Find<ModNPC>("TheScoundrelOfEarth_Tail").Type, NPC.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = NPC.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = NPC.whoAmI;
            ...
            if (NPC.ai[2] >= 150)  // 200 is npc spawn rate
            {
                ...
                cosmNPC = NPC.NewNPC((int)NPC.Center.X, (int)NPC.Center.Y, NPCID.GiantWormHead, NPC.whoAmI, 0, cosmNPC);
                cosmNPC = NPC.NewNPC((int)NPC.Center.X, (int)NPC.Center.Y, NPCID.DiggerHead, NPC.whoAmI, 0, cosmNPC);
                NPC.ai[2] = 0;
            }
