...
namespace AstralVoyage.NPCs.EoC
{
    ...
    public class TheEaterOfCosmos_Head : ModNPC
    {
        ...
        public override void BossLoot(ref string name, ref int potionType)
        {
            ...
            Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("UniversiteOre"), 56);
        ...
        public override bool PreAI()
        {
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("TheEaterOfCosmos_Body"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("TheEaterOfCosmos_Tail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
            ...
            if (npc.ai[2] >= 150)  // 200 is npc spawn rate
            {
                ...
                cosmNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("CosmosDrone"), npc.whoAmI, 0, cosmNPC);
                npc.ai[2] = 0;
            }
